      ...
      switch (animationState) {
        ...
        case "fadingIn": {
          ...
          particlesRef.current.forEach(particle => {
            ...
            const clr = particle.color.replace(/[\d.]+\)$/, `${opacity})`);
            ctx.fillStyle = clr;
...
const renderCanvas = ({
  ...
  const clr = parseColor(framerProps.color ?? "rgb(153, 153, 153)");
  ...
  const { particles, textBoundaries } = createParticles(ctx, canvas, currentText, textX, textY, fontStr, clr, framerProps.alignment || "left");
